    ...
    for n in range(num_buffs):
        ...
        with item_cols[1]:
            buff1level = st.selectbox(
                "Level",
                utils.class_for_name("set17buffs", buff1).levels,
                key=f"Buff_lvl_{n}_{champ_name}",
            )
        with item_cols[2]:
            extraParams = utils.class_for_name("set17buffs", buff1).extraParameters()
            if extraParams != 0:
...
def get_valid_powerups(champ, powerups):
    ...
    if champ.name in data:
        valid_powerups = data[champ.name] + set17buffs.no_buff
...
def total_takedowns(key, champ):
    ...
    takedowns = st.number_input(
        "Total takedowns", min_value=0, max_value=100, key=key
    )
...
def bonus_stats(key, champ):
    ...
    # Initialize defaults
    defaults = {
        key + "ad": 0, key + "dmgamp": 0, key + "manaregen": 0,
        key + "ap": 0, key + "crit": 0, key + "mpa": 0,
        key + "AS": 0, key + "critdmg": 0
    }
    ...
    with cols[0]:
        ad_bonus = st.number_input(
            "Bonus AD", min_value=0, max_value=2000, key=key + "ad"
        )
        dmgamp_bonus = st.number_input(
            "DmgAmp", min_value=0, max_value=1000, key=key + "dmgamp"
        )
        manaregen_bonus = st.number_input(
            "ManaRegen", min_value=0, max_value=10, key=key + "manaregen"
        )
    with cols[1]:
        ap_bonus = st.number_input(
            "Bonus AP", min_value=0, max_value=2000, key=key + "ap"
        )
        crit_bonus = st.number_input(
            "Bonus Crit", min_value=0, max_value=200, key=key + "crit"
        )
        mpa_bonus = st.number_input(
            "ManaPerAuto", min_value=0, max_value=15, key=key + "mpa"
        )
    with cols[2]:
        as_bonus = st.number_input(
            "Bonus AS", min_value=0, max_value=200, key=key + "AS"
        )
        crit_dmg_bonus = st.number_input(
            "CritDmg", min_value=0, max_value=200, key=key + "critdmg"
        )
...
def num_traits(key, champ):
    ...
    traits = st.number_input(
        "Number of active traits", min_value=0, max_value=12, key=key
    )
...
def add_buffs(champ, buffs, add_noitem=False):
    for (
        ...
        if buff != "NoBuff" or add_noitem:
            champ.items.append(
                utils.class_for_name("set17buffs", buff)(level, extraParams)
            )


def add_powerup(champ, powerup):
    champ.items.append(utils.class_for_name("set17powerups", powerup)(1, []))
...
def void_buff_selector(champion):
    ...
    void_buff_options = set17buffs.no_buff + set17buffs.void_buffs
...
def champ_selector(champ_list):
    ...
    with item_cols[1]:
        levels = [1, 2, 3]
        if utils.class_for_name("set17champs", champ).canFourStar:
            levels.append(4)
        champlevel = st.selectbox("Level", levels, index=0)

    new_champ = utils.class_for_name("set17champs", champ)(champlevel)
    return new_champ
