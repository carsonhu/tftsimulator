...
mana_items = set16items.mana_items + set16items.no_item
...
with st.sidebar:
    ...
    new_champ = set16champs.BaseChamp(1)
    ...
    for item in items:
        if item != 'NoItem':
            new_champ.items.append(utils.class_for_name('set16items', item)())
...
#     df_flt = df_flt[df_flt['Extra class name'].isin(set16items.artifacts+['NoItem'])]
# if radio_value == "Radiant":
#     df_flt = df_flt[df_flt['Extra class name'].isin(set16items.radiants+['NoItem'])]
# if radio_value == "Trait":
#     df_flt = df_flt[df_flt['Extra class name'].isin([x[0] for x in buffs]+['NoItem'])]
# if radio_value == "Augment/Buff":
#     df_flt = df_flt[df_flt['Extra class name'].isin(set16buffs.augments+ ['NoItem'])]
