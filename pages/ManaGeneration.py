...
mana_items = set18items.mana_items + set18items.no_item
...
with st.sidebar:
    ...
    new_champ = set18champs.BaseChamp(1)
    ...
    for item in items:
        if item != 'NoItem':
            new_champ.items.append(utils.class_for_name('set18items', item)())
...
#     df_flt = df_flt[df_flt['Extra class name'].isin(set18items.artifacts+['NoItem'])]
# if radio_value == "Radiant":
#     df_flt = df_flt[df_flt['Extra class name'].isin(set18items.radiants+['NoItem'])]
# if radio_value == "Trait":
#     df_flt = df_flt[df_flt['Extra class name'].isin([x[0] for x in buffs]+['NoItem'])]
# if radio_value == "Augment/Buff":
#     df_flt = df_flt[df_flt['Extra class name'].isin(set18buffs.augments+ ['NoItem'])]
