...
mana_items = set17items.mana_items + set17items.no_item
...
with st.sidebar:
    ...
    new_champ = set17champs.BaseChamp(1)
    ...
    for item in items:
        if item != 'NoItem':
            new_champ.items.append(utils.class_for_name('set17items', item)())
            new_champ.item_count += 1

    enemy = set17champs.ZeroResistance(1)
    champ_before_sims = copy.deepcopy(new_champ)
    set17_streamlit_main.doExperimentOneExtraWrapped.clear()
simLists, _ = set17_streamlit_main.doExperimentOneExtra(new_champ, enemy,
        utils.convertStrList('set17items', mana_items),
        [], t)
...
#     df_flt = df_flt[df_flt['Extra class name'].isin(set17items.artifacts+['NoItem'])]
# if radio_value == "Radiant":
#     df_flt = df_flt[df_flt['Extra class name'].isin(set17items.radiants+['NoItem'])]
# if radio_value == "Trait":
#     df_flt = df_flt[df_flt['Extra class name'].isin([x[0] for x in buffs]+['NoItem'])]
# if radio_value == "Augment/Buff":
#     df_flt = df_flt[df_flt['Extra class name'].isin(set17buffs.augments+ ['NoItem'])]
